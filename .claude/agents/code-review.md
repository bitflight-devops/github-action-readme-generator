...
- `vite.config.ts` — the authoritative lint config (Oxlint via Vite+'s `vp` CLI); run `vp check` rather than checking a rule against memory. Oxlint's own default/recommended rule set is used as-is — there's no repo-specific rule list here. One repo-specific override exists: `typescript/unbound-method` is disabled for `__tests__/**`, because the vitest-mock idiom reads a method reference rather than detaching and calling it; the reasoning is inline with the override
