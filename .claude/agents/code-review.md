...
### What NOT to Flag (Project Tolerances)

These are intentionally disabled in biome.json - do not flag as issues:
- `forEach` usage (noForEach: off)
- Computed property access (useLiteralKeys: off)
- `console` statements (noConsole: off)
- Parameter reassignment (noParameterAssign: off)
- Non-trailing default parameters (useDefaultParameterLast: off)
- Node.js import protocol (`node:` prefix not required)
