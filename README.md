...
contents is built before the usage section is rewritten, so it indexes whatever
usage block the previous run left behind. Run 2's contents catch up with run 1's
usage, and from run 2 onward there is nothing left to catch up on — a README
that shifts on the second run is showing you that, not a fault. In CI, generate
three times and compare passes 2 and 3; with an unchanged `action.yml` they are
identical.
