...
Write to that promise. It does not hold today with `pretty` on (the default),
which runs the **whole file** through prettier — tracked in #668, where which of
the two gives way is still open.
...
| **All formatting outside every marker**                                            | **prettier**                                        |
