...
- **Bootstrap `vp` itself in every workflow job that now calls it**: `vp`
  ...
  needs a qualifier, not a blanket removal**: it runs a 2-way version
  matrix (`node-version: ["24.19.0", "26.x"]`, confirmed by
  ...
  `node-version` input would collapse both matrix jobs onto
  whatever Node version `setup-vp` or the runner defaults to, silently
  losing the 24.19.0/26.x coverage `test.yml`'s own matrix exists
