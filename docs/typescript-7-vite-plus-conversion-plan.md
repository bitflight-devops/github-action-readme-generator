...
- **Bootstrap `vp` itself in every workflow job that now calls it**: `vp`
  ...
  to provide. `setup-vp`'s own `node-version` input takes the same kind
  of value `setup-node` does, so the fix is to pass
  `node-version: ${{ matrix.node-version }}` to `setup-vp` in this one
  workflow, not to omit the input.
